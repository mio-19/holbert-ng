...
type subst = Map.t<int, StringTerm.t>
...
let upshiftSubstVal = StringTerm.upshift
...
let parseSubstVal = StringTerm.parse

let prettyPrint = ((t, m): t, ~scope: array<StringTerm.meta>) =>
  `${StringTerm.prettyPrint(t, ~scope)} ${m}`
let prettyPrintSubstVal = (t: substVal, ~scope: array<StringTerm.meta>) => {
  `${StringTerm.prettyPrint(t, ~scope)}`
}

let mapSubst: (subst, substVal => substVal) => subst = Util.mapMapValues
