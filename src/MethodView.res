...
module type METHOD_VIEW = {
  ...
  module Method : PROOF_METHOD with 
    module Term := Term and 
  ...
  let make = (subRender : srProps<'a> => React.element) => (props) => {
      ...
      { props.method.subgoals->Array.mapWithIndex((sg,i) => {
          <li key={String.make(i)}>
            {React.createElement(subRender, {"proof":sg, "scope": props.scope, "ruleStyle": props.ruleStyle})}
          </li>
        })->React.array
      }
      </ul>
...
module LemmaView = (
  ...
  let make = (subRender : srProps<'a> => React.element) => (props) => {
      ...
      <RuleView 
        ...
        scope={props.scope} 
        style={props.ruleStyle}> {React.null}
      </RuleView>
      {React.createElement(subRender,{"proof":props.method.proof, "scope": props.scope, "ruleStyle": props.ruleStyle})}
      {React.createElement(subRender,{"proof":props.method.show, "scope": props.scope, "ruleStyle": props.ruleStyle})}
      </div>
  }
}
module CombineMethodView = (
  ...
  Method2View : METHOD_VIEW with module Term := Term and module Judgment := Judgment and type srProps<'a> = Method1View.srProps<'a>
) => {
  ...
  let make = (subrender : srProps<'a> => React.element) => (props) => {
    switch props.method {
    | First(m) => Method1View.make(subrender)({method:m, scope:props.scope, ruleStyle: props.ruleStyle})
    | Second(m) => Method2View.make(subrender)({method:m, scope:props.scope, ruleStyle: props.ruleStyle})
    }
  }
}