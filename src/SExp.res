...
let prettyPrintVar = (idx: int, scope: array<string>) => switch scope[idx] {
...
let rec prettyPrint = (it: t, ~scope: array<string>) => switch it{
| Symbol({name}) => name
...
| Schematic({schematic,allowed}) => 
  "?"->String.concat(String.make(schematic))
    ...
    ->String.concat(")")
| Compound({subexps}) =>
  "("->String.concat(Array.join(subexps->Array.map(e => prettyPrint(e,~scope=scope)), " "))
    ->String.concat(")")
}
